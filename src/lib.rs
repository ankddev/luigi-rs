...
pub use sys::{
    UI_ALIGN_LEFT, UI_ALIGN_RIGHT, UI_ALIGN_CENTER,
    UI_BUTTON_SMALL, UI_BUTTON_MENU_ITEM, UI_BUTTON_CAN_FOCUS, UI_BUTTON_DROP_DOWN, UI_BUTTON_CHECKED,
    UI_PANEL_HORIZONTAL, UI_PANEL_GRAY, UI_PANEL_WHITE, UI_PANEL_EXPAND, UI_PANEL_MEDIUM_SPACING,
    UI_PANEL_SMALL_SPACING, UI_PANEL_SCROLL, UI_PANEL_BORDER,
    UI_WINDOW_MENU, UI_WINDOW_INSPECTOR, UI_WINDOW_CENTER_IN_OWNER, UI_WINDOW_MAXIMIZE,
...
impl Window {
    ...
    pub fn new(title: &str, width: i32, height: i32, flags: u32) -> Result<Self> {
        ...
        let raw = unsafe { 
            sys::UIWindowCreate(ptr::null_mut(), flags, title.as_ptr(), width, height)
        };
        if raw.is_null() {
...
impl Button {
    ...
    pub fn new(parent: &impl Element, flags: u32, label: &str) -> Result<Self> {
        ...
        let raw = unsafe {
            sys::UIButtonCreate(parent.raw_element(), flags, label.as_ptr(), -1)
        };
        if raw.is_null() {
            return Err(Error::CreateFailed);
        }
...
impl Label {
    ...
    pub fn set_content(&self, text: &str) {
        let text = CString::new(text).map_err(|_| Error::InvalidString).unwrap_or_default();
        unsafe { 
...
impl Table {
    ...
    extern "C" fn message_handler(element: *mut sys::UIElement, message: i32, _di: i32, dp: *mut c_void) -> i32 {
        unsafe {
...
impl TextBox {
    ...
    pub fn get_text(&self) -> String {
        unsafe {
            let text = std::slice::from_raw_parts((*self.raw).string as *const u8, (*self.raw).bytes as usize);
...
impl Code {
    ...
    pub fn insert_content(&mut self, content: &str, replace: bool) {
        unsafe { 
            sys::UICodeInsertContent(self.raw, content.as_ptr() as *const i8, content.len() as isize, replace)
        }
    }
...
impl MDIChild {
    pub fn new(parent: &impl Element, flags: u32, bounds: sys::UIRectangle, title: &str) -> Result<Self> {
        let title = CString::new(title).map_err(|_| Error::InvalidString)?;
        let raw = unsafe { 
...
impl ColorPicker {
    ...
    pub fn get_color(&self) -> (f32, f32, f32, f32) {
        unsafe { 
            ((*self.raw).hue, (*self.raw).saturation, (*self.raw).value, (*self.raw).opacity)
        }
    }
...
impl ImageDisplay {
    pub fn new(parent: &impl Element, flags: u32, bits: &[u32], width: usize, height: usize) -> Result<Self> {
        let raw = unsafe { 
